...
class _DiaryPageState extends State<DiaryPage> {
  ...
  Future<void> _postDiary() async {
    final supabase = Supabase.instance.client; // Supabase 클라이언트 인스턴스 가져오기
    final user = supabase.auth.currentUser; // 현재 로그인한 사용자 가져오기
    final uuid = Uuid(); // UUID 생성기 인스턴스
  ...
  void initState() {
    super.initState();
    // 로그인 상태 확인
    final user = Supabase.instance.client.auth.currentUser; // Supabase 클라이언트에서 사용자 정보 가져오기
    if (user != null) {
      print('로그인된 사용자: ${user.id}');
    } else {
      print('사용자가 로그인되어 있지 않습니다.');
    }
  }
